...
function initDb() {
  db.prepare(
    'CREATE TABLE IF NOT EXISTS items (id INTEGER PRIMARY KEY, slug TEXT UNIQUE, title TEXT, content TEXT, date TEXT, image TEXT)'
  ).run();
  ...
  if (count === 0) {
    const insert = db.prepare(
      'INSERT INTO items (slug, title, content, date, image) VALUES (?, ?, ?, ?, ?)'
    );

    DUMMY_ITEMS.forEach((items) => {
      insert.run(
        items.slug,
        items.title,
        items.content,
